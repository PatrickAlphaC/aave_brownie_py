...
def get_borrowable_data(lending_pool, account):
    total_collateral_eth, total_debt_eth, available_borrow_eth, current_liquidation_threshold, tlv, health_factor = lending_pool.getUserAccountData(
        account.address)
...
def borrow_erc20(lending_pool, amount, account, erc20_address=None):
    erc20_address = erc20_address if erc20_address else config['networks'][network.show_active(
    )]['aave_link_token']
    # 1 is stable interest rate
    # 0 is the referral code
    transaction = lending_pool.borrow(erc20_address, Web3.toWei(amount, 'ether'), 1, 0,
